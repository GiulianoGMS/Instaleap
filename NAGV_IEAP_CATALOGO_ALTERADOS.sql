...
/*       LISTAGG(SEQCATEGORIAN1||CASE WHEN SEQCATEGORIAN2 IS NOT NULL THEN ', '||SEQCATEGORIAN2 END
       ...
       CASE WHEN C.CATEGORIAN1 = 'HORTIFRUTI' AND NOT EXISTS (SELECT 1 FROM MAP_PRODCODIGO E WHERE E.SEQPRODUTO = S.SEQPRODUTO AND E.TIPCODIGO = 'E') THEN NULL
            WHEN C.CATEGORIAN1 IN ('AÇOUGUE', 'PADARIA')    AND EXISTS (SELECT 1 FROM NAGT_DEPARA_STOCK_IEAP DP WHERE DP.SEQPRODUTO = S.SEQPRODUTO) THEN NULL
            ELSE CASE WHEN fc5estoquedisponivel(S.SEQPRODUTO,S.NROEMPRESA) < 0 THEN 0 ELSE fc5estoquedisponivel(S.SEQPRODUTO,S.NROEMPRESA) END END STOCK,
...
WHERE 1=1
 ...
 AND EXISTS (SELECT 1 FROM NAGV_IEAP_REGRA_PRODUTOS W WHERE W.seqproduto = S.SEQPRODUTO AND W.NROSEGMENTO = S.NROSEGMENTO AND W.NROEMPRESA = S.NROEMPRESA)
  --AND S.DTAVALIDACAOPRECO >= TRUNC(SYSDATE)-1
  --AND S.PRECOVALIDPROMOC = 0
	AND S.PRECOGERNORMAL > 0
