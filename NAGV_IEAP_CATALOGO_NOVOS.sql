...
WHERE 1=1
  ...
  AND X.ESTQLOJA                     >= NVL((SELECT PD.VALOR_PD FROM NAGT_PARAMETROS_IEAP PD WHERE PD.COD_PD = 'S'),              X.ESTQLOJA)
  --
  AND EXISTS (SELECT 1 FROM MAX_EMPRESASEG SG WHERE NROSEGMENTO IN (5,8) AND STATUS = 'A' AND SG.NROEMPRESA = X.NROEMPRESA)
  AND C.SEQCATEGORIAN1 != 40054 -- Removendo ROTISSERIE - Solic Roger via Teams / Giuliano / 24/03/25

GROUP BY X.SEQPRODUTO, X.NROEMPRESA, S.PRECOBASENORMAL,
         CASE WHEN C.CATEGORIAN1 = 'HORTIFRUTI' AND NOT EXISTS (SELECT 1 FROM MAP_PRODCODIGO E WHERE E.SEQPRODUTO = S.SEQPRODUTO AND E.TIPCODIGO = 'E') THEN 1000
            WHEN C.CATEGORIAN1 IN ('AÇOUGUE', 'PADARIA')    AND EXISTS (SELECT 1 FROM NAGT_DEPARA_STOCK_IEAP DP WHERE DP.SEQPRODUTO = S.SEQPRODUTO) THEN 1000
            ELSE CASE WHEN fc5estoquedisponivel(S.SEQPRODUTO,S.NROEMPRESA) < 0 THEN 0 ELSE fc5estoquedisponivel(S.SEQPRODUTO,S.NROEMPRESA) END END,
